...
try {
    ...
    if ($action === "list") {
        ...
        // Fetch all sessions
        $stmt = $db->query(
            "SELECT id, name, created_at, folder_id FROM sessions ORDER BY created_at DESC",
        );
        ...
        if ($type === "session") {
            $stmt = $db->prepare(
                "UPDATE sessions SET folder_id = :target WHERE id = :id",
            );
            $stmt->bindParam(
                ":target",
                $targetId,
                $targetId === null ? PDO::PARAM_NULL : PDO::PARAM_INT,
            );
            ...
            $stmt = $db->prepare(
                "UPDATE folders SET parent_id = :target WHERE id = :id",
            );
            $stmt->bindParam(
                ":target",
                $targetId,
        ...
        $updateStmt = $db->prepare(
            "UPDATE sessions SET folder_id = NULL WHERE folder_id = :id",
        );
        $updateStmt->bindParam(":id", $id, PDO::PARAM_INT);
        $updateStmt->execute();

        // Update subfolders to be in root (parent_id = NULL)
        $updateFolderStmt = $db->prepare(
        ...
        if ($id) {
            ...
            $stmt = $db->prepare(
                "UPDATE sessions SET name = :name, data = :data, folder_id = :folder_id, created_at = CURRENT_TIMESTAMP WHERE id = :id",
            );
            $stmt->bindParam(":name", $name);
            ...
            // Create new session
            $stmt = $db->prepare(
                "INSERT INTO sessions (name, data, folder_id) VALUES (:name, :data, :folder_id)",
            );
            $stmt->bindParam(":name", $name);
            $stmt->bindParam(":data", $data);
            $stmt->bindParam(":folder_id", $folderId);
        ...
        $id = $_GET["id"] ?? 0;
        $stmt = $db->prepare(
            "SELECT name, data, folder_id FROM sessions WHERE id = :id",
        );
        $stmt->bindParam(":id", $id);
        $stmt->execute();
