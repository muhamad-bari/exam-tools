...
try {
    ...
    // Header & Title
    $headerLine1 = isset($_POST["header_line1"])
        ? strtoupper(trim($_POST["header_line1"]))
        : "AKADEMI KEBIDANAN WIJAYA HUSADA";
    $headerLine2 = isset($_POST["header_line2"])
        ? strtoupper(trim($_POST["header_line2"]))
        : "";
    $subTitle = isset($_POST["sub_title"])
        ? trim($_POST["sub_title"])
        : "JADWAL UJIAN TENGAH SEMESTER (UTS) SEMESTER GENAP T.A 2024 / 2025";

    // Signer data
    $signer = [
        "nama" => isset($_POST["signer_name"])
            ? trim($_POST["signer_name"])
            : "Elpinaria Girsang, S.ST., M.K.M.",
        "jabatan" => isset($_POST["signer_title"])
            ? trim($_POST["signer_title"])
            : "Direktur",
        "institusi" => isset($_POST["signer_institution"])
            ? trim($_POST["signer_institution"])
            : "Akademi Kebidanan Wijaya Husada",
        "tanggal" => isset($_POST["signer_date"])
            ? trim($_POST["signer_date"])
            : date("d F Y"),
    ];
    ...
    $matkuls =
        isset($_POST["matkul"]) && is_array($_POST["matkul"])
            ? $_POST["matkul"]
            : [];
    $haris =
        isset($_POST["hari"]) && is_array($_POST["hari"]) ? $_POST["hari"] : [];
    $jams =
        isset($_POST["jam"]) && is_array($_POST["jam"]) ? $_POST["jam"] : [];
    $ruangs =
        isset($_POST["ruang"]) && is_array($_POST["ruang"])
            ? $_POST["ruang"]
            : [];
    ...
    while (($row = fgetcsv($handle, 10000, ",")) !== false) {
        ...
        if (
            !empty($data["nama"]) &&
            !empty(preg_replace("/[^0-9]/", "", $data["nim"]))
        ) {
    ...
    foreach ($students as $student) {
        // Generate QR code
        $qrContent =
            $student->nama . "_" . $student->tingkat . "_" . $student->nim;
        $qrData = "";
        ...
        try {
            ...
            if (file_exists($qrTempFile)) {
                $qrData =
                    ...
                    base64_encode(file_get_contents($qrTempFile));
                @unlink($qrTempFile);
            }
        } catch (Exception $e) {
        ...
        if ($logoData) {
            $logoHtml =
                '<img src="' .
                htmlspecialchars($logoData) .
                '" style="width: auto; max-width: 50px; max-height: 50px; position: absolute; left: 30px; top: 10px;">';
        }
        ...
        foreach ($schedule as $idx => $s) {
            $scheduleHtml .=
                '<tr>
                <td style="border: 1px solid black; padding: 3px 4px; text-align: center; font-size: 9pt;">' .
                ($idx + 1) .
                '</td>
                <td style="border: 1px solid black; padding: 3px 4px; font-size: 9pt;">' .
                htmlspecialchars($s["matkul"]) .
                '</td>
                <td style="border: 1px solid black; padding: 3px 4px; text-align: center; font-size: 9pt;">' .
                htmlspecialchars($s["hari"]) .
                '</td>
                <td style="border: 1px solid black; padding: 3px 4px; text-align: center; font-size: 9pt;">' .
                htmlspecialchars($s["jam"]) .
                '</td>
                <td style="border: 1px solid black; padding: 3px 4px; text-align: center; font-size: 9pt;">' .
                htmlspecialchars($s["ruang"]) .
                '</td>
        ...
        $htmlBody .=
            ...
            ($headerLine2
                ...
                : "") .
            '
                    <h4 style="margin: 5px 0 0 0; font-weight: normal; font-size: 10pt;">' .
                    ...
                    <table style="width: 100%; border: none;">
                        <tr>
                            <td style="vertical-align: top;">
                                <table style="width: 100%; border: none;">
                                    <tr><td style="width: 70px;">KELAS</td><td>: ' .
            ...
            '
                ...
                <div style="margin-top: 10px; text-align: center; width: 45%; float: right;">
                    <p style="margin-bottom: 0; font-size: 9pt;">Bogor, ' .
            htmlspecialchars($signer["tanggal"]) .
            '</p>
                    <p style="margin-bottom: 40px; font-size: 9pt;">Mengetahui<br>' .
            htmlspecialchars($signer["institusi"]) .
            "<br>" .
            htmlspecialchars($signer["jabatan"]) .
            '</p>
                    <p style="font-weight: bold; text-decoration: underline; font-size: 9pt;">' .
            htmlspecialchars($signer["nama"]) .
            '</p>
                </div>
    ...
    echo json_encode([
        ...
        "filename" =>
            "Jadwal_Ujian_" . $csvBaseName . "_" . date("YmdHis") . ".pdf",
    ]);
