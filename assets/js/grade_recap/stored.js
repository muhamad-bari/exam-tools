function renderStoredClassList(items, meta = {}) {
  ...
  if (!items || !items.length) {
    summary.textContent = activeBits.length
      ? `Belum ada data rekap untuk filter: ${activeBits.join(" • ")}`
      : "Belum ada data kelas dengan rekap nilai tersimpan.";
    body.innerHTML =
      '<tr><td colspan="6" style="text-align:center; color:#64748b;">Belum ada data rekap tersimpan.</td></tr>';
    return;
  }
...
async function loadStoredClassRecaps(showToastOnSuccess = false) {
  ...
  try {
    const response = await fetch(
      `index.php?api=rekap_nilai&${params.toString()}`,
    );
...
function renderClassSubjectRows(items, meta = {}) {
  ...
  const activeFilters = meta.active_filters || {};
  const filterBits = [
    activeFilters.exam_type,
    activeFilters.term,
    activeFilters.academic_year,
  ].filter(Boolean);
  ...
  if (!classSubjectRows.length) {
    body.innerHTML =
      '<tr><td colspan="6" style="text-align:center; color:#64748b;">Belum ada mata kuliah tersimpan untuk kelas/filter ini.</td></tr>';
    cancelClassSubjectEdit();
    return;
  }

  body.innerHTML = classSubjectRows
    .map((row, index) => {
...
function setClassSubjectsLoading(message = "Memuat mata kuliah kelas...") {
  ...
  if (body) {
    body.innerHTML =
      '<tr><td colspan="6" style="text-align:center; color:#64748b;">Memuat data...</td></tr>';
  }
}
...
function openClassSubjectsModal(encodedClassName) {
  ...
  const loadSubjectsPromise =
    Array.isArray(availableSubjects) && availableSubjects.length
      ? Promise.resolve()
      : loadSubjectOptions();
...
function startClassSubjectEdit(index) {
  ...
  if (datalist) {
    datalist.innerHTML = availableSubjects
      .map(
        (subject) =>
          `<option value="${escapeHtml(subject.name || "")}"></option>`,
      )
      .join("");
  }
...
function getSelectedClassSubjectEditSubject() {
  ...
  const selectedById = selectedId
    ? availableSubjectsById.get(selectedId) || null
    : null;
...
async function saveClassSubjectEdit() {
  ...
  try {
    ...
    const response = await fetch(
      "index.php?api=rekap_nilai&action=update_class_subject",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
async function deleteClassSubjectValues(index) {
  ...
  if (
    !confirm(
      `Hapus nilai ${label} untuk kelas "${currentClassSubjectModalClassName}"?`,
    )
  ) {
    return;
  }
  ...
  try {
    const response = await fetch(
      "index.php?api=rekap_nilai&action=delete_class_subject_values",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
    ...
    if (!result.success) {
      throw new Error(
        result.message || "Gagal menghapus nilai mata kuliah kelas",
      );
    }
    ...
    await loadClassSubjectRows();
    showToast(
      `${result.deleted_rows || 0} baris nilai mata kuliah berhasil dihapus`,
    );
  } catch (error) {
    showToast(error.message, "error");
  }
