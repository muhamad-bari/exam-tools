function initGradeRecapPage() {
  document
    .getElementById("gradeFile")
    .addEventListener("change", updateSelectedFileInfo);

  const subjectSearchInput = document.getElementById("subjectSearchInput");
  const subjectSuggestions = document.getElementById(
  ...
  const getSubjectSuggestionMatches = () => {
    const query = String(subjectSearchInput?.value || "")
      .trim()
      .toLowerCase();
  ...
  const setActiveSubjectSuggestion = (index) => {
    ...
    const items = Array.from(
      subjectSuggestions.querySelectorAll(".subject-suggestion"),
    );
    if (!items.length) {
      activeSubjectSuggestionIndex = -1;
      return;
    ...
    activeSubjectSuggestionIndex = Math.min(
      Math.max(index, 0),
      items.length - 1,
    );
    items.forEach((item, itemIndex) => {
      item.classList.toggle(
        "active",
        itemIndex === activeSubjectSuggestionIndex,
      );
    });
  ...
  const chooseSubjectSuggestion = (subjectId) => {
    ...
    if (previousSubjectId && previousSubjectId !== nextSubjectId) {
      clearSelectedGradeFiles(
        true,
        "File Excel dibersihkan karena mata kuliah berubah. Pilih file yang sesuai mata kuliah baru.",
      );
    }
  ...
  const renderSubjectSuggestions = () => {
    ...
    subjectSuggestions.innerHTML = matches
      .map(
        (subject) => `
  ...
  subjectSearchInput.addEventListener("keydown", (event) => {
    ...
    if (event.key === "ArrowDown") {
      ...
      event.preventDefault();
      setActiveSubjectSuggestion(
        activeSubjectSuggestionIndex <= 0
          ? items.length - 1
          : activeSubjectSuggestionIndex - 1,
      );
    } else if (event.key === "Enter" && activeSubjectSuggestionIndex >= 0) {
      event.preventDefault();
      chooseSubjectSuggestion(
        items[activeSubjectSuggestionIndex].dataset.subjectId,
      );
  ...
  document.getElementById("examTypeSelect").addEventListener("change", () => {
    ...
    if (previousExamType && previousExamType !== selectedExamType) {
      clearSelectedGradeFiles(
        true,
        "File Excel dibersihkan karena jenis ujian berubah.",
      );
    }
  });

  document
    .getElementById("academicYearInput")
    .addEventListener("change", () => {
      ...
      if (
        ...
        previousAcademicYear !== selectedAcademicYear
      ) {
        clearSelectedGradeFiles(
          true,
          "File Excel dibersihkan karena tahun ajaran berubah.",
        );
      }
    });
  ...
  document.getElementById("termSelect").addEventListener("change", () => {
    ...
    if (previousTerm && previousTerm !== selectedTerm) {
      clearSelectedGradeFiles(
        true,
        "File Excel dibersihkan karena periode semester berubah.",
      );
    }
  });

  document
    .getElementById("storedExamTypeFilter")
    .addEventListener("change", () => {
      syncStoredFiltersFromDom();
      loadStoredClassRecaps(false);
    });
  document
    .getElementById("storedAcademicYearFilter")
    .addEventListener("change", () => {
      syncStoredFiltersFromDom();
      loadStoredClassRecaps(false);
    });
  ...
  Promise.all([loadSubjectOptions(), loadAcademicPeriodOptions()])
    .then(() => {
      syncSelectedSubjectFromDom();
      updateUploadButtonState();
      setConfirmedSubjectId(
        selectedSubject ? Number(selectedSubject.id) : null,
      );
