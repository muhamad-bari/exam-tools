function renderAssignRows() {
  ...
  if (!rows.length) {
    body.innerHTML =
      '<tr><td colspan="5" style="text-align:center; color:#64748b;">Semua mahasiswa sudah cocok dengan master.</td></tr>';
    return;
  }
...
function toggleAssignSelectAll(checked) {
  ...
  assignSelectedNims = new Set(
    checked ? rows.map((item) => String(item.nim)) : [],
  );
  renderAssignRows();
}
...
function openAssignClassModal() {
  ...
  loadClassOptions()
    .then(() => {
      document.getElementById("assignModalInfo").textContent =
        `${rows.length} mahasiswa belum ada di master. Pilih kelas tujuan lalu simpan.`;
...
async function submitAssignClass() {
  ...
  const selectedRows = getUnmatchedRows().filter((item) =>
    assignSelectedNims.has(String(item.nim)),
  );
  ...
  for (const row of selectedRows) {
    try {
      const response = await fetch(
        "index.php?api=master_data&action=create_student",
        {
          method: "POST",
  ...
  if (failedCount > 0) {
    ...
    renderAssignRows();
    showToast(
      `${failedCount} mahasiswa gagal diproses. Baris gagal sudah di-highlight untuk retry.`,
      "error",
    );
  } else {
    closeAssignClassModal();
