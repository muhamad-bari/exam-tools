...
function updateClearGradeFileButtonState() {
  ...
  clearBtn.style.display = getSelectedGradeFiles().length
    ? "inline-flex"
    : "none";
}

function clearSelectedGradeFiles(
  showToastMessage = false,
  toastMessage = "File Excel terpilih sudah dibersihkan.",
) {
  const input = document.getElementById("gradeFile");
  if (!input) {
    return false;
...
function renderInactiveStudentsModal(rows) {
  ...
  if (!inactiveRows.length) {
    ...
    info.textContent =
      "Mahasiswa nonaktif akan muncul di sini jika terdeteksi pada template upload.";
    return;
  }
...
function uploadGradeRecap() {
  ...
  if (!academicYear) {
    showToast(
      "Pilih tahun ajaran dari master periode akademik terlebih dahulu",
      "error",
    );
    return;
  }
  ...
  fetch("index.php?api=rekap_nilai", { method: "POST", body: formData })
    ...
    .then((res) => {
      ...
      const resolvedAcademicYear =
        res?.meta?.academic_year || academicYear || "-";
      const resolvedTerm = res?.meta?.term || term || "-";
      const uploadedCount = Number(
        res?.meta?.uploaded_file_count || files.length || 1,
      );
      const primaryFileName =
        res?.meta?.file_name || files[0]?.name || "nilai.xlsx";
      const summaryFileText =
        uploadedCount > 1
          ? `${uploadedCount} file (${primaryFileName} + lainnya)`
          : primaryFileName;
      const inactiveCount = Number(res?.summary?.inactive_students || 0);
      document.getElementById("summaryInfo").textContent =
        `${summaryFileText} - ${subjectName} (${resolvedExamType}, ${resolvedTerm}, ${resolvedAcademicYear}) - Sheet ${res.meta.sheet_name} (ikut rekap: ${res.summary.total_rows || 0}, unik NIM: ${res.summary.unique_nim_rows || 0}, nonaktif: ${inactiveCount})`;
      document.getElementById("detectedInfo").textContent =
        `Kolom tetap digunakan untuk sheet ${res.meta.sheet_name}. Upload terbaru akan menjadi recap aktif untuk ${subjectName} (${resolvedExamType}, ${resolvedTerm}, ${resolvedAcademicYear}) pada kelas yang sama. Mahasiswa status cuti/keluar tidak ikut rekap sampai diaktifkan kembali di Master Data.`;
      document.getElementById("totalRows").textContent = String(
        res.summary.total_rows || 0,
      );
      document.getElementById("avgNormal").textContent = formatNumber(
        res.summary.avg_normal,
      );
      document.getElementById("avgRemedial").textContent = formatNumber(
        res.summary.avg_remedial,
      );
      document.getElementById("avgSusulan").textContent = formatNumber(
        res.summary.avg_susulan,
      );
      document.getElementById("avgFinal").textContent = formatNumber(
        res.summary.avg_final,
      );
      document.getElementById("matchedStudents").textContent = String(
        res.summary.matched_students || 0,
      );
      document.getElementById("unmatchedStudents").textContent = String(
        res.summary.unmatched_students || 0,
      );
      document.getElementById("inactiveStudents").textContent =
        String(inactiveCount);
      document.getElementById("duplicateRows").textContent = String(
        res.summary.duplicate_nim_rows || 0,
      );
