function renderPagedTable() {
    ...
    if (!allGradeRows.length) {
        body.innerHTML = '<tr><td colspan="10" style="text-align:center; color:#64748b;">Belum ada data rekap nilai.</td></tr>';
...
function renderFixedColumns(columns) {
    ...
    if (!columns || !Object.keys(columns).length) {
        container.innerHTML = '<span class="muted">Belum ada konfigurasi ditampilkan.</span>';
        return;
    }

    container.innerHTML = Object.entries(columns).map(([key, value]) => `<span class="pill"><i class="fa-solid fa-table-columns"></i> ${escapeHtml(key)}: <strong>${escapeHtml(value)}</strong></span>`).join('');
}
...
function resetGradeRecap() {
    ...
    document.getElementById('summaryInfo').textContent = 'Upload file nilai untuk melihat ringkasan nilai murni, SP, dan susulan.';
