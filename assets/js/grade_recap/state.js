...
function syncSelectedSubjectFromDom() {
  ...
  if (searchInput) {
    const normalizedName = String(searchInput.value || "")
      .trim()
      .toLowerCase();
    if (!normalizedName) {
      select.value = "";
...
function renderUploadAcademicYearOptions(preferredValue = "") {
  ...
  const uniqueYears = Array.from(
    new Set(masterAcademicPeriods.map((item) => item.academic_year)),
  );
  const preservedValue = normalizeAcademicYearValue(
    preferredValue || select.value || selectedAcademicYear || "",
  );
...
function renderUploadTermOptions(academicYear, preferredValue = "") {
  ...
  const preservedValue = String(
    preferredValue || select.value || selectedTerm || "",
  )
    .trim()
    .toUpperCase();
...
function syncStoredFiltersFromDom() {
  ...
  const academicYearSelect = document.getElementById(
    "storedAcademicYearFilter",
  );
  const termSelect = document.getElementById("storedTermFilter");

  storedFilters = {
    examType: examTypeSelect
      ? String(examTypeSelect.value || "")
          .trim()
          .toUpperCase() || null
      : null,
    academicYear: academicYearSelect
      ? normalizeAcademicYearValue(academicYearSelect.value) || null
      : null,
    term: termSelect
...
function applyStoredFilterMeta(meta = {}) {
  ...
  setSelectOptions(
    "storedExamTypeFilter",
    options.exam_types || ["UTS", "UAS"],
    "Semua Jenis Ujian",
  );
  setSelectOptions(
    "storedAcademicYearFilter",
...
function getUnmatchedRows() {
  return allGradeRows.filter(
    (item) => String(item.master_match_type || "") === "not_found",
  );
}

function getInactiveRows() {
  return allGradeRows.filter(
    (item) => String(item.master_match_type || "") === "inactive",
  );
}
...
function refreshMasterStats() {
  const matched = allGradeRows.filter(
    (item) => String(item.master_match_type || "") === "active",
  ).length;
...
function buildDistributionFromRows() {
  ...
  participatingRows.forEach((item) => {
    const name =
      (item.kelas || item.master_class || "Tanpa kelas").trim() ||
      "Tanpa kelas";
